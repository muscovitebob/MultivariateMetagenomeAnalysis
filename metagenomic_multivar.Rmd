...
ggpairs(PCA2_and_desc, aes(colour=Status), columns=1:7)
```
...
```{r}
library(MASS)
separated_test = test[,1] %>% separate(SampleInfo, into=c("Sample", "Dataset", "Status"), sep="~")
train_status = bind_cols(train[-1], dplyr::select(separated, Status))
test_status = bind_cols(test[-1], dplyr::select(separated_test, Status))
lda1 = lda(Status~., data=train_status)
predict_lda1 = predict(lda1, test_status)
```
