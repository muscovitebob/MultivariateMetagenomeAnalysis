...
#PCA2scores = as.matrix(train[-1]) %*% as.matrix(PCA2$rotation)
...
PCA2_and_desc = bind_cols(as_tibble(PCA2$x), separated)
ggpairs(PCA2_and_desc, aes(colour=Status), columns=1:7, progress = FALSE)
```

```{r}
ggpairs(PCA2_and_desc, aes(colour=Dataset), columns=1:7, progress = FALSE)
```
...
constrainedAbundances1 = bind_cols(separated_X[, 1:3],as.tibble(constrainedModel1$Ybar))
tsne_constrained1 = Rtsne(constrainedAbundances1, dims=2, perplexity=30, max_iter=400)
tsne_constrained1 = as_tibble(tsne_constrained1$Y)
tsne_constrained1_with_additionals = bind_cols(separated_X[,1:3], tsne_constrained1)
ggplot(tsne_constrained1_with_additionals, aes(x=V1, y=V2,colour=Dataset)) + geom_point() 
```

```{r}
constrainedModel2 = cca(separated_X[, -c(1,2,3)] ~ Status + Condition(Dataset), data=separated_X)
constrainedAbundances2 = bind_cols(separated_X[, 1:3],as.tibble(constrainedModel2$Ybar))
tsne_constrained2 = Rtsne(constrainedAbundances2, dims=2, perplexity=30, max_iter=400)
tsne_constrained2 = as_tibble(tsne_constrained2$Y)
tsne_constrained2_with_additionals = bind_cols(separated_X[,1:3], tsne_constrained2)
ggplot(tsne_constrained2_with_additionals, aes(x=V1, y=V2,colour=Dataset)) + geom_point() 
```
...
Exploration of a model fitted using Correspondence Analysis via tSNE reveals previously unseen structure in the data that better accounts for clinical status.
